    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">SimulationEnvironment已Preparation Complete，canStartRunningSimulation</p>
          
          <!-- Simulationroundscountconfiguration - 只YesinconfigurationGeneratingCompleted且roundscount计算出come后才显示 -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">SimulationroundscountSetting</span>
                <span class="section-desc">MiroFish AutomaticPlanningDeductionReality <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents real-world <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes time elapsed</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If agent scale is100：Estimated time约 {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">IfFirst timeRunning，StronglyRecommendSwitch to‘Custom模式’ReduceSimulationroundscount，In order toQuickPreviewEffectAndReduceErrorRisk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              ...
              <span class="info-value">{{ { male: '男', female: '女', other: '其he' }[selectedProfile.gender] || selectedProfile.gender }}</span>
            </div>
            <div class="info-item">
          ...
          <div class="modal-section">
            <span class="section-label">ProfileBio</span>
            <p class="section-bio">{{ selectedProfile.bio || 'No Bio Available' }}</p>
          </div>

          <!-- Focus onTopic -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">RealitySeedAssociatedTopic</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            <span class="section-label">DetailedProfile背景</span>
            
            <!-- Profile维度概览 -->
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">事件panorama经历</span>
                <span class="dim-desc">in此事件中ofcomplete行for轨迹</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">行for模式侧写</span>
                <span class="dim-desc">经验Total结with行事风格偏好</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">独特记忆印记</span>
                <span class="dim-desc">基于RealitySeed形成of记忆</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">社willRelations网络</span>
                <span class="dim-desc">units体链接withInteractionGraph</span>
              </div>
            </div>
...
// State
const phase = ref(0) // 0: Initializing, 1: GeneratingProfile, 2: Generatingconfiguration, 3: Completed
...
// loggo重：record上一次输出of关keyInfo
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulationroundscountconfiguration
const useCustomRounds = ref(false) // 默认Use automatic configurationroundscount
const customMaxRounds = ref(40)   // 默认Recommendation40rounds
...
watch(currentStage, (newStage) => {
  if (newStage === 'GeneratingAgentProfile' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // EnterconfigurationGenerating阶段，Startrounds询configuration
    if (!configTimer) {
      addLog('StartGenerate Dual-Platform Simulation configuration...')
      startConfigPolling()
    }
  } else if (newStage === '准备Simulation脚本' || newStage === 'copying_scripts') {
    phase.value = 2 // 仍属于configuration阶段
  }
})

// fromconfiguration中计算AutomaticGeneratingofroundscount（not使use硬编码默认value）
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // configuration未Generating时Back null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // configurationdatanotcomplete时Back null
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // 确保maximumroundscountnot小于40（Recommendationvalue），避免滑动items范围异常
...
// according toagent_idgetto应ofusername
...
// 计算所YesProfileofAssociatedTopicTotal
...
// ProcessStart SimulationbuttonClick
const handleStartSimulation = () => {
  // Building传递give父组件ofparameters
  const params = {}
  
  if (useCustomRounds.value) {
    // use户Customroundscount，传递 max_rounds parameters
    params.maxRounds = customMaxRounds.value
    addLog(`Start Simulation，Customroundscount: ${customMaxRounds.value} rounds`)
  } else {
    // use户选择保持AutomaticGeneratingofroundscount，not传递 max_rounds parameters
    addLog(`Start Simulation，Use automatic configurationroundscount: ${autoGeneratedRounds.value} rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 立即SettingsExpected Total Agents（fromprepareinterfaceBackvalueget）
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`fromZepGraphRead到 ${res.data.expected_entities_count} unitsEntity`)
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ Entity Types: ${res.data.entity_types.join(', ')}`)
        }
      }
      
      addLog('Startrounds询准备进度...')
      // Startrounds询进度
      startPolling()
      // StartReal-timeget Profiles
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // parse阶段InfoAnd输出Detailedlog
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage_name || ''
        
        // 输出Detailed进度log（避免重复）
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出 Profile Generation Progresslog（仅当quantityChange时）
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 输出configurationGenerating阶段log（避免重复）
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('GeneratingAgentProfileconfiguration...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('In progress调useLLMGeneratingSimulationconfigurationparameters...')
        }
      }
      
      // ifconfiguration已Generating
      if (data.config_generated && data.config) {
        simulationConfig.value = data.config
        addLog('✓ SimulationconfigurationGeneratingCompleted')
        
        // 显示Detailedconfiguration摘want
        if (data.summary) {
        ...
        if (data.config.event_config?.narrative_direction) {
          ...
          addLog(`NarrativeDirection: ${narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative}`)
        }
...
const loadPreparedData = async () => {
  ...
  // getconfiguration（使useReal-timeinterface）
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // configuration尚未Generating，Startrounds询
        addLog('configurationGenerating，Startrounds询Wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(`LoadingconfigurationFailed: ${err.message}`)
    emit('update-status', 'error')
  }
}
...
/* Simulationroundscountconfiguration样式 */
